...
export function hubHead(hub: HubDef) {
  ...
  return {
    ...
    scripts: (() => {
      ...
      const steps = hub.sections.flatMap((s) => s.steps || []);
      if (steps.length > 0) {
        scriptsArr.push({
          type: "application/ld+json",
          children: JSON.stringify({
    ...
    <PageContainer>
      ...
      {hub.sections.map((s) => (
        <section key={s.h2} className="mt-10 max-w-3xl">
          ...
          {s.table && (
            <div className="overflow-x-auto mb-6 mt-4 rounded-lg border border-border">
              <table className="w-full text-left text-sm whitespace-nowrap">
                <thead className="bg-muted/50 border-b border-border">
                  <tr>
                    {s.table.head.map((h, i) => (
                      <th key={i} className="px-4 py-3 font-semibold text-foreground">{h}</th>
                    ))}
                  </tr>
                </thead>
          ...
          {s.steps && (
            <div className="space-y-4 mb-6 mt-4">
              {s.steps.map((step, i) => (
                <div key={i} className="flex gap-4 p-4 rounded-lg border border-border bg-surface">
                  <div className="flex-shrink-0 size-8 rounded-full bg-primary/10 text-primary flex items-center justify-center font-bold font-display">
                    {i + 1}
                  </div>
                  <div>
