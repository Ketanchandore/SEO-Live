...
import { Check, ChevronRight, HelpCircle, PlayCircle, Network, ArrowRight } from "lucide-react";
...
export function ToolContent() {
  ...
  const loadedMeta = leafMatch?.loaderData as ToolMeta | undefined;
  
  const path = useRouterState({ select: (s) => s.location.pathname });
  const slug = path.split("/tools/")[1]?.split("/")[0];
  const meta = loadedMeta ?? (slug ? getToolMeta(slug) : undefined);
  ...
  return (
    <>
      <section className="mt-12 max-w-4xl mx-auto space-y-12 pb-32">
        {/* Quick Facts Box */}
        ...
        {/* Video SEO & Multimodal Search Block */}
        <Block id="video-guide" title={`Video Guide: How to Use ${tn}`}>
          <p className="text-muted-foreground mb-4">
            Watch our quick 2-minute video tutorial on how to get the most out of this tool for your website optimization.
          </p>
          <div className="relative w-full aspect-video bg-surface-2 border border-border rounded-xl overflow-hidden group cursor-pointer shadow-lg">
            <div className="absolute inset-0 bg-black/40 flex items-center justify-center transition-all group-hover:bg-black/20 z-10">
               <PlayCircle className="size-16 text-white drop-shadow-md opacity-80 group-hover:scale-110 transition-transform" />
            </div>
            <img 
              src="https://images.unsplash.com/photo-1460925895917-afdab827c52f?q=80&w=2015&auto=format&fit=crop" 
              alt="SEO Tutorial Thumbnail" 
              className="w-full h-full object-cover opacity-60"
            />
            <div className="absolute bottom-4 left-4 z-10 text-white font-semibold drop-shadow-md">
              Tutorial: Mastering {tn} in 2026
            </div>
          </div>
        </Block>
        ...
        {/* Visual SEO Architecture Block */}
        <Block id="architecture" title={`How The Engine Works`}>
          <p className="text-muted-foreground mb-4">Our serverless architecture fetches real-time data without caching, ensuring you see exactly what Googlebot sees right now.</p>
          <div className="w-full p-8 rounded-xl border border-border bg-gradient-to-br from-surface to-surface-2 flex items-center justify-between text-center">
             <div className="flex flex-col items-center gap-2">
                <div className="size-12 rounded-full bg-primary/10 flex items-center justify-center"><Network className="size-6 text-primary" /></div>
                <div className="text-sm font-semibold">Live URL Fetch</div>
             </div>
             <ArrowRight className="size-5 text-muted-foreground" />
             <div className="flex flex-col items-center gap-2">
                <div className="size-12 rounded-full bg-primary/10 flex items-center justify-center"><Check className="size-6 text-primary" /></div>
                <div className="text-sm font-semibold">Semantic Parsing</div>
             </div>
             <ArrowRight className="size-5 text-muted-foreground" />
             <div className="flex flex-col items-center gap-2">
                <div className="size-12 rounded-full bg-primary/10 flex items-center justify-center"><Check className="size-6 text-primary" /></div>
                <div className="text-sm font-semibold">SEO Scoring</div>
             </div>
          </div>
        </Block>
        ...
        <div className="rounded-xl border border-border bg-surface-2 p-5 text-sm text-muted-foreground flex flex-wrap gap-4 items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="size-10 bg-primary/20 rounded-full flex items-center justify-center font-bold text-primary">SA</div>
            <div>
