...
export function detectTech(html: string, headers: Record<string, string>) {
  ...
  if (/gtag\(|google-analytics/i.test(html)) add("Google Analytics", "Analytics", "gtag");
  if (/googletagmanager\.com\/gtm\.js/i.test(html)) add("Google Tag Manager", "Analytics", "gtm.js");
  if (/hotjar\.com|hj\(|hjid/i.test(html)) add("Hotjar", "Analytics", "hotjar script");
  if (/intercom\.io|Intercom\(/i.test(html)) add("Intercom", "Support", "intercom script");
  if (/fbevents\.js|fbq\(/i.test(html)) add("Meta Pixel", "Advertising", "fbevents");
  if (/tiktok\.com\/awq|ttq\./i.test(html)) add("TikTok Pixel", "Advertising", "tiktok events");
  if (/snap\.licdn\.com|linkedin_data_partner_id/i.test(html)) add("LinkedIn Insight", "Advertising", "linkedin tag");
...
export function countSyllables(w: string) {
