...
export const Route = createFileRoute("/tools/image-seo")({
  loader: () => getToolContent("image-seo"),
    ...
    <>
      ...
      <Section title={`All images (${imgs.length})`}>
        <div className="max-h-96 overflow-auto">
          <table className="w-full text-xs"><thead><tr className="text-left text-muted-foreground"><th>Preview</th><th>Src</th><th>Alt</th><th>Loading</th><th>Format</th></tr></thead><tbody>
            {imgs.map((im,i) => {
              const extMatch = im.src.match(/\.([a-z0-9]+)(?:[?#]|$)/i);
              const format = extMatch ? extMatch[1].toLowerCase() : "unknown";
              const isModern = /webp|avif|svg/.test(format);
              return (
                <tr key={i} className="border-t border-border/60">
                  ...
                  <td className={`py-1 ${isModern ? "text-success" : ""}`}>{format}</td>
                </tr>
              );
            })}
          </tbody></table>
