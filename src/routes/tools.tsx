...
export const Route = createFileRoute("/tools")({
  head: () => ({
    meta: [
      ...
      { property: "og:image", content: `${SITE_URL}/blog-og.webp` },
      { property: "og:title", content: "SEOAcademys Tools Dashboard" },
      { property: "og:description", content: "Free AI-SEO tools dashboard. Citation audit, schema, keyword research, SERP preview, backlinks, and more." },
    ],
...
function SidebarNav({ path, collapsed, onNavigate }: { path: string; collapsed: boolean; onNavigate?: () => void }) {
  return (
    <nav className="flex-1 min-h-0 overflow-y-auto md:overflow-visible px-2 pb-6 space-y-5">
      {NAV.map((group) => (
        <div key={group.label}>
          {!collapsed && (
        ...
        <script
          ...
          dangerouslySetInnerHTML={{
            __html: JSON.stringify({
              ...
              "name": `${NAMES[slug] ?? "SEO Tool"} - SEOAcademys`,
              ...
              "itemListElement": [
                {
                  ...
                  "position": 1,
                  "name": "Tools",
                  "item": "https://seoacademys.com/tools"
                },
                {
                  "@type": "ListItem",
                  "position": 2,
                  "name": NAMES[slug] ?? "Tool",
                  "item": `https://seoacademys.com/tools/${slug}`
                }
              ]
            })
          }}
