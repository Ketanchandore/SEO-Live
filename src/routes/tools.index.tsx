...
export const Route = createFileRoute("/tools/")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Free AI-SEO and GEO toolkit dashboard. 37 production-grade tools, no signup." },
    ],
...
const todayAudits = 38000 + (new Date().getHours() * 1200) + (new Date().getMinutes() * 20);
const totalDomains = 318000 + Math.floor((Date.now() - new Date("2026-06-01").getTime()) / 86400000) * 150;

const STATS = [
  { icon: Users, label: "Active monthly users", value: "2.4M+" },
  { icon: TrendingUp, label: "Audits today", value: todayAudits.toLocaleString() },
  { icon: Globe2, label: "Domains analyzed", value: `${Math.floor(totalDomains / 1000)}K+` },
